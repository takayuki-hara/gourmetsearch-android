...
class GourmetApiClient {
    fun gourmetSearch(keyword: String, area: String, loaded: (GourmetSearchResponse?) -> Unit) {
        val key = loadApiKey()
        if (key == null) {
            loaded(null)
            return
        ...
        provider.gourmetSearch(key = key, keyword = keyword, largeArea = area, format = "json").enqueue(object : Callback<GourmetSearchResponse> {
